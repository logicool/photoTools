...
@interface PhotoTweakView () <UIScrollViewDelegate, CropViewDelegate, UIGestureRecognizerDelegate>
             ...
             maxRotationAngle:(CGFloat)maxRotationAngle
{
    if (self = [super init]) {
        ...
        _photoContentView.userInteractionEnabled = NO; //@@@TODO YES->NO
        ...
        _cropView = [[CropView alloc] initWithFrame:CGRectMake(self.scrollView.frame.origin.x, self.scrollView.frame.origin.y, self.scrollView.frame.size.width, self.scrollView.frame.size.width * 0.75)];
        ...
        [self sliderValueChanged:0]; // @适配裁切框初始为4：3比例问题；
    }
    return self;
}
...
- (void)sliderValueChanged:(float) angle
{
    // update masks
    [self updateMasks:NO];
    ...
    // rotate scroll view
    self.angle += angle;
    self.scrollView.transform = CGAffineTransformRotate(self.scrollView.transform, angle);
